...
public static class SlowQueryMarkdownReportGenerator
{
    ...
    public static string GenerateReport(ISlowQueryRanking ranking, int topN = 20)
    {
        ...
        if (allSuggestions.Count > 0)
        {
            ...
            report.AppendLine();
            report.AppendLine("| Table | Columns | Reason | Include Columns | Suggested | Total Duration | SQL Hint |");
            report.AppendLine("|-------|---------|--------|----------------|-----------|----------------|----------|");

            foreach (var suggestion in allSuggestions)
            {
                var sqlHint = suggestion.Suggestion.ToSqlHint();
                ...
                report.AppendLine($"| `{suggestion.Table}` | `{string.Join(", ", suggestion.Columns)}` | " +
                    ...
                    $"{suggestion.Count:N0} times | " +
                    $"{suggestion.TotalDurationMs:N0} ms | " +
                    $"`{truncatedHint}` |");
            }
